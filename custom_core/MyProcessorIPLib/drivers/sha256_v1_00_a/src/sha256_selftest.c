/*****************************************************************************
* Filename:          /home/arlininger/FPGA/ZYBO/custom_core/MyProcessorIPLib/drivers/sha256_v1_00_a/src/sha256_selftest.c
* Version:           1.00.a
* Description:       Contains a diagnostic self-test function for the sha256 driver
...
#define SHA256_SELFTEST_BUFSIZE  4 /* Size of buffer (n bytes) is fixed to 4 for lite Master */
#define SHA256_USER_NUM_REG  32/* Number of registers in slave */

/************************** Variable Definitions ****************************/

static Xuint8 __attribute__((aligned (128))) SrcBuffer[SHA256_SELFTEST_BUFSIZE];   /* Source buffer      */
static Xuint8 __attribute__((aligned (128))) DstBuffer[SHA256_SELFTEST_BUFSIZE];   /* Destination buffer */
...
XStatus SHA256_SelfTest(void * baseaddr_p)
{
  ...
  for (write_loop_index = 0 ; write_loop_index < SHA256_USER_NUM_REG; write_loop_index++)
    SHA256_mWriteSlaveReg0 (baseaddr, write_loop_index*4, (write_loop_index+1)*READ_WRITE_MUL_FACTOR);
  for (read_loop_index = 0 ; read_loop_index < SHA256_USER_NUM_REG; read_loop_index++)
    if ( SHA256_mReadSlaveReg0 (baseaddr, read_loop_index*4) != (read_loop_index+1)*READ_WRITE_MUL_FACTOR){
  ...
  if ( (SHA256_mReadSlaveReg0 (baseaddr, 0)) != 0x0){
