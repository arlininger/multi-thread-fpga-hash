/*****************************************************************************
* Filename:          /home/arlininger/FPGA/ZYBO/custom_core/MyProcessorIPLib/drivers/sha256_v1_00_a/src/sha256.c
* Version:           1.00.a
* Description:       sha256 Driver Source File
...
void SHA256_MasterSendWord(Xuint32 BaseAddress, Xuint32 DstAddress)
{
   ...
   */
  Xil_Out8(BaseAddress+SHA256_MST_CNTL_REG_OFFSET, MST_SGWR);

  /*
   * Set user logic master address register to drive IP2Bus_Mst_Addr signal.
   */
  Xil_Out32(BaseAddress+SHA256_MST_ADDR_REG_OFFSET, DstAddress);

  /*
   * Set user logic master byte enable register to drive IP2Bus_Mst_BE signal.
   */
  Xil_Out16(BaseAddress+SHA256_MST_BE_REG_OFFSET, 0xFFFF);

  /*
   * Start user logic master write transfer by writting special pattern to its go port.
   */
  Xil_Out8(BaseAddress+SHA256_MST_GO_PORT_OFFSET, MST_START);
}
...
void SHA256_MasterRecvWord(Xuint32 BaseAddress, Xuint32 SrcAddress)
{
   ...
   */
  Xil_Out8(BaseAddress+SHA256_MST_CNTL_REG_OFFSET, MST_SGRD);

  /*
   * Set user logic master address register to drive IP2Bus_Mst_Addr signal.
   */
  Xil_Out32(BaseAddress+SHA256_MST_ADDR_REG_OFFSET, SrcAddress);
